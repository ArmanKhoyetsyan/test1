...
const step1 = async (ctx) => {
  let text = '';
  let num = 1;
  Object.entries(PERCENT_OBJ['CHEQUE PAYPAL']).forEach(([key, value]) => {
    if (value) {
      text += `<b>${(num)}</b> ${key}\n`;
      num += 1;
    }
...
step2.on('text', async (ctx) => {
  try {
    ...
    if (number && number <= 6) {
      ...
      ctx.wizard.state.platform = PERCENT_OBJ['CHEQUE PAYPAL ARR'][number - 1];
      ctx.wizard.next();
...
const secondSceneSecondAnswerThirdSceneSecondAnswer = new WizardScene(
...
module.exports = { secondSceneSecondAnswerThirdSceneSecondAnswer };
