...
const step1 = async (ctx) => {
  let text = '';
  let num = 1;
  Object.entries(PERCENT_OBJ['SALDO PAYPAL']).forEach(([key, value]) => {
    if (value) {
      text += `<b>${(num)}</b> ${key}\n`;
      num += 1;
    }
...
step2.on('text', async (ctx) => {
  try {
    ...
    if (number && number <= 6) {
      ...
      ctx.wizard.state.platform = PERCENT_OBJ['SALDO PAYPAL ARR'][number - 1];
      ctx.wizard.next();
...
const thirdServiceTwoSceneCalculator = new WizardScene(
...
module.exports = { thirdServiceTwoSceneCalculator };
